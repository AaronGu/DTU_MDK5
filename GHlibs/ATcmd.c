...
{
    for(uint8_t i=0;i<AT_RSP_CMD_MAX_NUM;i++)
        if(at.at_rsp[i].len)
        {
...
void at_send_cmd(char *fmt,...)
{
    ...
    //HalUSARTWriteData(AT_UART_PORT,(uint8_t *)string,strlen(string));
		HAL_UART_Transmit(AT_UART_PORT,(uint8_t *)string,strlen(string), 100);
    va_end(ap);
    Free(string);
}
//发送数据
void at_send_data(uint8_t *buf,uint16_t len)
{
    //HalUSARTWriteData(AT_UART_PORT,buf,len);
	  HAL_UART_Transmit(AT_UART_PORT, buf, len, 100);
}
...
{
    uint8_t i;                                               /* 索引 */
    if(len < AT_BUF_LEN_MAX)                                 /* 检查接收数据是否超出缓冲级长度上线 */
    {
